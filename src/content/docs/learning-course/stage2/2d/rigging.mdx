---
title: Rigging
description: Understanding rigging in elevators
---

import ContentFigure from '@components/content/ContentFigure.astro';

## Rigging

Elevator rigging typically takes up the bulk of the design process, as its the most important part of the elevator. Rigging style, and organization drives the motor mounting, and position of the crossmember the clamp plate is mounted to, so its important to properly plan out the cable paths.

<ContentFigure src="../img/2d/rigging-light.webp" alt="Example of a 3 stage cascade elevator rigging" width="50%">Example of a 3 stage cascade elevator rigging</ContentFigure>
