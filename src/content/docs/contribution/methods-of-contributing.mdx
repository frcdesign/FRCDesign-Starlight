...
In [the discord server](https://discord.gg/frcdesign), go to the "website-discussion" channel and ask if you can contribute the thing you want to by filling out this template:
...
If you're given approval to start working on it, an internal contributor will make a new post in the "public-website-contribution" forum channel or direct you to an existing post to work with and supplement another contributor if the content or issue has already started to be addressed.
...
If you create a new `.mdx` file you must also add an entry for it in `src/config/sidebarConfig.ts`, otherwise the page will exist but won't appear in the navigation. Find the section that matches where your page lives and add a line like `{ label: 'Your Page Title', slug: 'section/your-filename' }`. See the [Style Guide](/contribution/styleguide#adding-a-new-page) for full details.
</Aside>
